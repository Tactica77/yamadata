    ...
    public void createGpxDatas(){
        ...
        for( TrackPoint tp: gpx.getTrackPoints().values() ){
            ...
            if ( beforetp.ele < tp.ele ){
                ...
                time_all += TrackLib.distanceSec( beforetp, tp );
        }
            if ( beforetp.ele > tp.ele ){
                // 下り
                hd_d += m;
        ...
        this.overwriteYamaData( "gpxTime", time_all + "" );    // 山行時間
    }
}
